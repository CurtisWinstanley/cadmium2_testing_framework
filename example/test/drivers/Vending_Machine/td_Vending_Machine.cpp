...
// Model Under Test
#include "top_Vending_Machine.hpp"
...
int main() {
	...
	for(int test_set_enumeration = 1; test_set_enumeration <= test_set_size; test_set_enumeration++)
	{
		...
		log_file_dir = log_file_dir + std::string("/") + std::string("Vending_Machine");
		...
		auto td = std::make_shared<top_Vending_Machine>("Top_"+model_under_test_name, test_set_enumeration, log_file, t, c, p);
